...
;(async () => {
  ...
  const types = ['LIKE'] // 'FIRE', 'LAUGH', 'MINDBLOWN', 'LIKE'
  // 点赞用户凭证
  const cookies = [
    {
      ...
      if (galleryInfo && galleryInfo.data) {
        ...
        for (let i = 0; i < galleries.length; i++) {
          for (let j = 0; j < types.length; j++) {
            // if (i < 31) {
            //   const rs = await view(cookie, galleries[i].id, deviceId)
            // }
            console.log('🚀 ~ file: index.js ~ line 256 ~ addLike ~ cookie, types[j], galleries[i].id, deviceId', cookie, types[j], galleries[i].id, deviceId)
