...
const galleryCount = 50;

const cookie = '__Host-next-auth.csrf-token=5ba3626cbde4d7ff7d438dc662a5c2ebb54c034b67d3cb719d678cc0eaef14c3%7Cfc1b5a77acbc213463389506e019acd14f3bdb8a34a9935b77b2903bad68e6a5; __Secure-next-auth.callback-url=https%3A%2F%2Fdeca.art; _hp2_ses_props.3997967461=%7B%22ts%22%3A1659620541920%2C%22d%22%3A%22deca.art%22%2C%22h%22%3A%22%2Fdecagon%2Fdxp%22%2C%22ubv%22%3A%22103.0.5060.134%22%2C%22upv%22%3A%2214.0.0%22%7D; _hp2_id.3997967461=%7B%22userId%22%3A%224345641330196707%22%2C%22pageviewId%22%3A%221320088381711029%22%2C%22sessionId%22%3A%221489663619572408%22%2C%22identity%22%3Anull%2C%22trackerVersion%22%3A%224.0%22%7D; __Secure-next-auth.session-token=eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIn0..tZg0nS1ZGOaPXK51.hycd1s_h4EznczHWCUfu9HcD-ExXRYhAJebMqNTxqflBxY1V-VfMwLu-kOjKW-4_e7Kf8WtnR-z6TNqzQawKcutcwe68SAnLXYklapamM7Gb_zz_V2CMUKrHmharLm7-hBsRQ4ROU8yZEs64rRAHfvpQq4SdWhNrSgeMGXGnf4u0qwrFksSNKyzLioMurKLri4RMXw3q3C6YFbrY4yB6wk5ZN7qs0m5z.q5vLIfufD4UFE-P3kKi7qQ'
;(async () => {
  const getNfts = (cookie) => {
    const res = axios({
      ...
      data: {
        query: '\n    query SearchNfts($searchSpace: SearchSpace!, $query: String!, $contract: String, $tokenId: String, $address: String, $showEth: Boolean!, $showTez: Boolean!, $showBtc: Boolean!, $showPoly: Boolean!) {\n  searchNfts(\n    searchSpace: $searchSpace\n    query: $query\n    contract: $contract\n    tokenId: $tokenId\n    address: $address\n    showEth: $showEth\n    showTez: $showTez\n    showBtc: $showBtc\n    showPoly: $showPoly\n  ) {\n    id\n    tokenId\n    contract\n    provider\n    mediaUrl\n    previewStorageKey\n    name\n  }\n}\n    ',
        variables: {
          query: "",
          searchSpace: "MINE",
          showBtc: true,
  ...
  for(let d = 0; d < decaList.length; d++) {
    for(let i = 0; i < galleryCount; i++) {
      ...
      const res = await generateGallery(decaList[d].cookie, galleryName)
      if (res && res.data && res.data.data.createGallery) {
        const resIdInfo = await getGalleryId(decaList[d].cookie, galleryName, decaList[d].usernameOrAddress)
        if (resIdInfo && resIdInfo.data && resIdInfo.data.data.gallery) {
          ...
          const nfts = await getNfts(decaList[d].cookie)
          if (nfts && nfts.data && nfts.data.data.searchNfts) {
            ...
            const result = await createGallery(decaList[d].cookie, sectionsId, galleryId, nftInfo)
            if (result && result.data && result.data.data.updateGallery) {
              console.log(`第${d + 1}个用户的--${galleryName}--创建成功`)
            }
          }
        }
      }
  ...
  const res = await createGallery(cookie).catch(e => console.log(e))
  if (res && res.data) {
    console.log('🚀 ~ file: createGallery.js ~ line 154 ~ ; ~ res.data', res.data)
  }

  // for (let d = 0; d < decaList.length; d++) {
  //   const galleryInfo = await getNfts(decaList[d].cookie).catch(err => {})
  //   if (galleryInfo && galleryInfo.data) {
  //     const info = galleryInfo.data
  //     const firstNft = info.data.searchNfts[0]
